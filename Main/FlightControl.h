...
void FlightControl(){
  ...
  if((millis()-ts)>1){  //Update only once per 2ms (500hz update rate)
    ...
    if(throttle > 2){ 
      roll.compute( 0 , SetPoint[0] + SetPointOptMain[0],throttle, -xOpt.output*innerEffect  +  Trim_Roll +Trim_Roll_Bs, -attitude[1], rate[1]); 
      pitch.compute( 1 , SetPoint[1] + SetPointOptMain[1] ,throttle, -yOpt.output*innerEffect +  Trim_Pitch+  Trim_Pitch_Bs , -attitude[0], -rate[0]);
